...
| 2 | Rename `legacy_lub_assoc_holds` to `legacy_lub_associativity` | After a clean 118-job preflight, the corrected gate rejects with `✗ missing theorem: legacy_lub_assoc_holds in Tgrad/Dtype.lean`. This theorem is explicitly limited to L1's historical 14-member fixture; the pinned current relation has 24 compiler-counted non-associative triples. | scripts/gates/L1.sh | ✓ 2026-07-29 |
