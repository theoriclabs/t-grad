...
./.lake/build/bin/tgrad-cli rangeify "$TGRAD_DIR/fixtures/schedule/rangeify_input.json" \
    >/tmp/tgrad_L2_rangeify.json 2>&1 || {
  echo "  ✗ tgrad-cli rangeify failed"; cat /tmp/tgrad_L2_rangeify.json; exit 1
}
if ! diff -q /tmp/tgrad_L2_rangeify.json "$TGRAD_DIR/fixtures/schedule/rangeify_expected.json" >/dev/null; then
  echo "  ✗ Tgrad.Schedule.Rangeify disagrees with captured chain output"
  diff /tmp/tgrad_L2_rangeify.json "$TGRAD_DIR/fixtures/schedule/rangeify_expected.json" | head -20
  exit 1
fi
...
./.lake/build/bin/tgrad-cli mem-plan "$TGRAD_DIR/fixtures/schedule/intervals.json" \
    >/tmp/tgrad_L2_mem.json 2>&1 || {
  echo "  ✗ tgrad-cli mem-plan failed"; cat /tmp/tgrad_L2_mem.json; exit 1
}
if ! diff -q /tmp/tgrad_L2_mem.json "$TGRAD_DIR/fixtures/schedule/assignment_expected.json" >/dev/null; then
  echo "  ✗ Tgrad.Schedule.Memory disagrees with captured assignment"
  diff /tmp/tgrad_L2_mem.json "$TGRAD_DIR/fixtures/schedule/assignment_expected.json" | head -20
  exit 1
fi
...
./.lake/build/bin/tgrad-cli schedule "$TGRAD_DIR/fixtures/schedule/detailed_schedule.json" \
    >/tmp/tgrad_L2_sched.json 2>&1 || {
  echo "  ✗ tgrad-cli schedule failed"; cat /tmp/tgrad_L2_sched.json; exit 1
}
if ! diff -q /tmp/tgrad_L2_sched.json "$TGRAD_DIR/fixtures/schedule/detailed_schedule.json" >/dev/null; then
  echo "  ✗ Tgrad.Schedule.Linear disagrees with captured detailed schedule"
  diff /tmp/tgrad_L2_sched.json "$TGRAD_DIR/fixtures/schedule/detailed_schedule.json" | head -20
  exit 1
fi
...
EOF
if (cd "$TGRAD_DIR" && lake env lean /tmp/tgrad_neg_L2.lean) >/tmp/tgrad_neg_L2.log 2>&1; then
  echo "  ✗ negative test compiled — type system isn't enforcing SinkItem.functionName"
  cat /tmp/tgrad_neg_L2.log
  exit 1
fi
...
rangeify_hash="$(shasum -a 256 /tmp/tgrad_L2_rangeify.json | awk '{print $1}')"
mem_hash="$(shasum -a 256 /tmp/tgrad_L2_mem.json | awk '{print $1}')"
sched_hash="$(shasum -a 256 /tmp/tgrad_L2_sched.json | awk '{print $1}')"
