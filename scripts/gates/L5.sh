...
(cd "$REPO_ROOT" && "$TGRAD_DIR/.lake/build/bin/tgrad-cli" matmul --shape 64x64x64 --dtype bf16) \
    >/tmp/tgrad_L5_matmul.txt 2>&1 || {
  echo "  ✗ tgrad-cli matmul --shape 64x64x64 --dtype bf16 failed"
  cat /tmp/tgrad_L5_matmul.txt; exit 1
}
...
for token in "compile_ok: 1" "alloc_ok: 1" "dispatch_rc: 0" "pipeline_ok: 1"; do
  if ! grep -q "$token" /tmp/tgrad_L5_matmul.txt; then
...
(cd "$REPO_ROOT" && "$TGRAD_DIR/.lake/build/bin/tgrad-cli" matmul-verify --shape 64x64x64 --seed 42) \
    >/tmp/tgrad_L5_verify.txt 2>&1 || {
  echo "  ✗ tgrad-cli matmul-verify failed"
  cat /tmp/tgrad_L5_verify.txt; exit 1
}
grep -q "matmul_verify_ok: 1" /tmp/tgrad_L5_verify.txt || {
...
matmul_hash="$(shasum -a 256 /tmp/tgrad_L5_matmul.txt | awk '{print $1}')"
verify_hash="$(shasum -a 256 /tmp/tgrad_L5_verify.txt | awk '{print $1}')"
