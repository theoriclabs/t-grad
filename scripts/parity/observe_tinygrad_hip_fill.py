#!/usr/bin/env python3
"""Observe pinned tinygrad HIP lowering without compiler or hardware.

This foreign observer never supplies expected values to Tgrad.  It bypasses
HIPRenderer's compiler-bearing initializer and records source/launch facts
from lazy tinygrad lowering at the exact pinned oracle revision.
"""
...
def main() -> int:
    ...
    if observations[0]["scalar_f32_bits"] == observations[1]["scalar_f32_bits"]:
        raise SystemExit("sensitivity failure: changed value kept same float bits")
        ...
        json.dumps(
            {
                ...
                "intentional_difference": (
                    "pinned tinygrad selects exact global*local extent and no guard; "
                    "the shared Tgrad fill plan uses caller-admitted block size, "
                    "ceil-div groups, and idx<element_count guarded overdispatch. "
                    "Both cover exactly [0, element_count), but their source and "
                    "launch identities are deliberately not claimed equal"
                ),
                "sensitivity": (
                    "PASS_changed_count_value_dtype_changed_foreign_source_launch_bits"
                ),
            },
            indent=2,
            sort_keys=True,
